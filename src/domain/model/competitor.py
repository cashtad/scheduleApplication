...
class Competitor(Human):
    ...
    def __post_init__(self) -> None:
        ...
        if self.amount_of_people == 2 and (self.dancer_2_name is None or not self.dancer_2_name.strip()):
            raise ValueError("dancer_2_name cannot be empty when amount_of_people is 2")
        if self.amount_of_people == 1 and self.dancer_2_name is not None and self.dancer_2_name.strip():
